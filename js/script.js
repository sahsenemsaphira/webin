...
const tunnelTl = gsap.timeline({
    scrollTrigger: {
        ...
        start: "top top",
        end: "+=2000", // Scroll mesafesi (Animasyon süresi)
        pin: true,
        scrub: 1.5,
    }
});
...
// ----------------------------------------------------------------
// 1. MASAÜSTÜ (Desktop) - 800px ve üzeri
// ----------------------------------------------------------------
mm.add("(min-width: 800px)", () => {
    ...
    const storyTl = gsap.timeline({
        scrollTrigger: {
            ...
            start: "top top",
            end: "+=2600", // Masaüstünde uzun scroll mesafesi
            pin: true,      // Sadece masaüstünde sabitle
            scrub: 1,
            // markers: true // Test ederken açabilirsin
        }
    });
    ...
    steps.forEach((step, i) => {
        ...
        // Öncekini Söndür (Son adım hariç)
        if (i < steps.length - 1) {
    ...
    return () => {
        // Temizlik (Cleanup) - Opsiyonel ama iyi pratik
        // Masaüstü modundan çıkınca timeline'ı öldürür.
    };
});

// ----------------------------------------------------------------
// 2. MOBİL (Mobile) - 799px ve altı
// ----------------------------------------------------------------
mm.add("(max-width: 799px)", () => {

    // Mobilde PIN YOK, karmaşık timeline YOK.
    // Sadece aşağı indikçe "Fade In" olsunlar.

    steps.forEach((step, i) => {
        gsap.fromTo(step,
            { opacity: 0.2, y: 50 }, // Başlangıç hali
            {
                opacity: 1,
                y: 0,
                ...
                scrollTrigger: {
                    ...
                    scrub: false, // Scrub yok, animasyon bir kere oynasın
                    toggleActions: "play none none reverse"
                }
            }
        );

        // Resimler mobilde yazının altındaysa veya üstündeyse onlar da ayrı tetiklensin
        if (images[i]) {
            gsap.fromTo(images[i],
                { opacity: 0, scale: 0.9 },
                {
                    ...
                    duration: 0.8,
                    scrollTrigger: {
                        trigger: images[i], // Resmi görünce tetikle
                        start: "top 85%",
                        toggleActions: "play none none reverse"
                    }
                }
